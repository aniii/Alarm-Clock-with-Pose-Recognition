...
-(bool)processFinger:(int *)redAvgValue andGreen:(int *)greenAvgValue andBlue:(int *)blueAvgValue isFlashOn:(bool)isFlashOn is100Frames:(bool)is100Frames{
    cv::Mat image_copy;
    char text[50];
    ...
    // avgPixelIntensity.val[0] is R,
    // avgPixelIntensity.val[2] is B
    if (is100Frames) {
        cv::putText(_image, "100 Frames!!!", cv::Point(0, 100), FONT_HERSHEY_PLAIN, 0.75, Scalar::all(255), 1, 2);
    } else {
    ...
    *redAvgValue = avgPixelIntensity.val[0];
    *greenAvgValue = avgPixelIntensity.val[1];
    *blueAvgValue = avgPixelIntensity.val[2];
    
    
    // if avgPixelIntensity.val[0]< 50 and avgPixelIntensity.val[2] < 5 and avgPixelIntensity.val[3] < 5, then finger is on the camera
    // return true
    return !isFlashOn ? avgPixelIntensity.val[0] < 50 && avgPixelIntensity.val[1] < 5 &&avgPixelIntensity.val[2] < 5 :
        avgPixelIntensity.val[0] > 190 && avgPixelIntensity.val[1] < 20 && avgPixelIntensity.val[2] < 60 ;
}
